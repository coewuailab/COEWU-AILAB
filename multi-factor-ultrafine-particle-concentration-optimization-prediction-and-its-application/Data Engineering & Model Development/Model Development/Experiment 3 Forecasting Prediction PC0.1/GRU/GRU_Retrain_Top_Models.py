"""
ปรับปรุงโค้ดสำหรับการเทรนซ้ำโมเดล GRU และลดความซับซ้อนของโครงสร้างไฟล์
...
# สมมติว่า import จากไฟล์เดิมที่ชื่อ GRU_Bayesian.py
from GRU import (
    GRUModel, EarlyStopping, prepare_data_expanding_window,
...
class RetrainTopModels:
    """
    คลาสสำหรับการเทรนโมเดล GRU ที่ดีที่สุดซ้ำหลายรอบ
    ...
    def __init__(self,
        ...
        if output_dir is None:
            ...
            self.output_dir = f"PM01_GRU_Retrain_Top{num_top_models}_{timestamp}"
        else:
    ...
    def train_model_once(self, hyperparams, model_rank, retrain_idx, models_dir, logs_dir, plots_dir,
        ...
        # สร้างชื่อโมเดล
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_name = f"GRU_Rank{model_rank}_Retrain{retrain_idx}_{timestamp}"
        ...
        # บันทึกโมเดลและ log
        # บันทึกโมเดล
        model_filename = f"GRU_Rank{model_rank}_Retrain{retrain_idx}.pth"
...
if __name__ == "__main__":
    ...
    NUM_RETRAIN = 10  # เปลี่ยนเป็น 10 รอบตามความต้องการ
    NUM_TOP_MODELS = 10  # เปลี่ยนเป็น 10 โมเดลตามความต้องการ

    # สร้างโฟลเดอร์สำหรับเก็บผลลัพธ์
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    OUTPUT_DIR = f"PM01_GRU_Retrain_Top{NUM_TOP_MODELS}_{timestamp}"
