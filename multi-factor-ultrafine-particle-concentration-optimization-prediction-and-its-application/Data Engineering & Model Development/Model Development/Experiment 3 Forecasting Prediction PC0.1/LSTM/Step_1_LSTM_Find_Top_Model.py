"""
การเทรนโมเดล LSTM ด้วย PyTorch สำหรับทำนายค่า PM0.1 พร้อมระบบ Bayesian Hyperparameter Tuning
...
# สำหรับเปลี่ยน type จาก numpy to json
def default_converter(obj):
    if isinstance(obj, np.integer):
        ...
        return obj.__getstate__() if hasattr(obj, '__getstate__') else str(obj)
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        # กำหนดค่าเริ่มต้นของ hidden state และ cell state
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(x.device)
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(x.device)

        # ส่งข้อมูลผ่านชั้น LSTM
...
def prepare_data_expanding_window(file_path, numeric_features, categorical_features, sequence_length,
    ...
    # สกัดข้อมูล target
    y_data = df['PM0.1'].values.reshape(-1, 1)
...
def plot_results_expanding(train_losses, val_losses, test_actuals, test_predictions,
    ...
    plt.title(f'LSTM Model Loss - Window {window_index + 1}')
    plt.ylabel('Loss')
    plt.xlabel('Epochs')
    plt.legend()
...
def save_model_and_logs_expanding(model, optimizer, train_loss, val_loss, input_size, output_size,
    ...
    """
    บันทึกโมเดล LSTM และข้อมูลการเทรนสำหรับเทคนิค Expanding Window
    """
    # บันทึกโมเดล
    ...
    with open(log_file, 'w', encoding='utf-8') as f:
        f.write("========== LSTM Model Training Log (Expanding Window) ==========\n\n")
...
def run_bayesian_optimization(data_args, fixed_params, tuning_dir, n_calls=120):
    """
    ทำการ Bayesian Hyperparameter Tuning สำหรับโมเดล LSTM
...
def train_top_models(data_args, fixed_params, top_models, base_dir):
    ...
    for i, model_info in enumerate(top_models):
        ...
        # สร้างชื่อโมเดล
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_name = f"LSTM_Rank{rank}_seq{params['sequence_length']}_h{params['hidden_size']}_l{params['num_layers']}_{timestamp}"
        ...
        model_summary = {
            ...
            'avg_losses_plot_path': avg_losses_plot_path,  # ไม่จำเป็นต้องใช้เงื่อนไขเพราะประกาศตัวแปรแล้ว
            'n_windows': len(all_windows_results)
        }
...
def run_bayesian_tuning_and_train_top_models(csv_file_path, fixed_params=None, n_calls=120):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_dir = "PM01_LSTM_Bayesian_Tuning"
...
if __name__ == "__main__":
    ...
    file_path = 'Merge_81Days_1min_with_daytypes.csv'  # แก้ไขเป็นพาธของไฟล์ของคุณ
