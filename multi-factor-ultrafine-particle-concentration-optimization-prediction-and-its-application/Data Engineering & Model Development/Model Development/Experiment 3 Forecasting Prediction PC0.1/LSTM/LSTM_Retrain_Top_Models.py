"""
ปรับปรุงโค้ดสำหรับการเทรนซ้ำโมเดล LSTM และลดความซับซ้อนของโครงสร้างไฟล์
...
# import จากไฟล์เดิมที่ชื่อ LSTM_Bayesian.py
from LSTM_Bayesian import (
    LSTMModel, EarlyStopping, prepare_data_expanding_window,
...
class RetrainTopModels:
    ...
    def __init__(self,
        ...
        if output_dir is None:
            ...
            self.output_dir = f"PM01_LSTM_Retrain_Top{num_top_models}_{timestamp}"
        else:
    ...
    def train_model_once(self, hyperparams, model_rank, retrain_idx, models_dir, logs_dir, plots_dir,
        ...
        # สร้างชื่อโมเดล
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_name = f"LSTM_Rank{model_rank}_Retrain{retrain_idx}_{timestamp}"
        ...
        # บันทึกโมเดลและ log
        # บันทึกโมเดล
        model_filename = f"LSTM_Rank{model_rank}_Retrain{retrain_idx}.pth"
